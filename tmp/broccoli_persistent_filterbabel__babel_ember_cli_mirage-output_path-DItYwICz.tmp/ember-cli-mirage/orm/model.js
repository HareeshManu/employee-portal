  ...
  class Model {
    ...
    */
    ...
    _validateAttrs(attrs) {
      ...
      Object.keys(attrs).filter(key => this.associationKeys.includes(key)).forEach(key => {
        let value = attrs[key];
        let association = this.associationFor(key);
        let isNull = value === null;

        if (association instanceof _hasMany.default) {
          let isCollection = value instanceof _collection.default || value instanceof _polymorphicCollection.default;
          let isArrayOfModels = Array.isArray(value) && value.every(item => item instanceof Model);

          (0, _assert.default)(isCollection || isArrayOfModels || isNull, `You're trying to create a ${this.modelName} model and you passed in "${value}" under the ${key} key, but that key is a HasMany relationship. You must pass in a Collection, PolymorphicCollection, array of Models, or null.`);
        } else if (association instanceof _belongsTo.default) {
          (0, _assert.default)(value instanceof Model || isNull, `You're trying to create a ${this.modelName} model and you passed in "${value}" under the ${key} key, but that key is a BelongsTo relationship. You must pass in a Model or null.`);
        }
      });

      // Verify attrs passed in for association foreign keys are actually fks
      Object.keys(attrs).filter(key => this.associationIdKeys.includes(key)).forEach(key => {
        let value = attrs[key];

        if (key.match(/Ids$/)) {
          let isArray = Array.isArray(value);
          let isNull = value === null;
          (0, _assert.default)(isArray || isNull, `You're trying to create a ${this.modelName} model and you passed in "${value}" under the ${key} key, but that key is a foreign key for a HasMany relationship. You must pass in an array of ids or null.`);
        }
      });
      ...
      if (Array.isArray(foreignKeys)) {
        ...
        if (association.isPolymorphic) {
          found = foreignKeys.map(({ type, id }) => {
            return this._schema.db[(0, _normalizeName.toInternalCollectionName)(type)].find(id);
          });
          found = (0, _compact2.default)(found);
        } else {
          found = this._schema.db[(0, _normalizeName.toInternalCollectionName)(association.modelName)].find(foreignKeys);
        }
        ...
        let found;
        if (association.isPolymorphic) {
          found = this._schema.db[(0, _normalizeName.toInternalCollectionName)(foreignKeys.type)].find(foreignKeys.id);
        } else {
          found = this._schema.db[(0, _normalizeName.toInternalCollectionName)(association.modelName)].find(foreignKeys);
        }
      ...
      if (tempAssociation && associateIds) {
        ...
        if (association.isPolymorphic) {
          models = associateIds.map(({ type, id }) => {
            return this._schema[(0, _normalizeName.toCollectionName)(type)].find(id);
          });
        } else {
    ...
    */
    ...
    _associateModelWithInverse(model, association) {
      if (model.hasInverseFor(association)) {
        ...
        if (inverse instanceof _belongsTo.default) {
          ...
          this._schema.db[(0, _normalizeName.toInternalCollectionName)(model.modelName)].update(model.id, { [inverseFk]: newId });
        } else {
          let inverseCollection = this._schema.db[(0, _normalizeName.toInternalCollectionName)(model.modelName)];
    ...
    _updateInDb(attrs) {
      this.attrs = this._schema.db[(0, _normalizeName.toInternalCollectionName)(this.modelName)].update(this.attrs.id, attrs);
    }
